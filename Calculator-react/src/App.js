...
function App() {
  const [display, setDisplay] = useState('0'); //Adding a default state to the display

  const handleNumber = (event) => { //Handling the number input
  ...
  const handleOperator = (event) => { //Handling the operator input
    const operator = event.target.textContent;

    setDisplay(display + ' ' + operator + ' ');
  };

  const handleEquals = () => { //Calculating the result
    setDisplay(eval(display));

  };

  const handleDecimal = () => { //Handling the decimal input
  ...
  const handleClear = () => { //Clearing the display
    setDisplay('0');
  };
  ...
  return (
    <div className="App">
      <div className="calculator">
        ...
        <div id="equals" className="long_row"  onClick={handleEquals}>=</div>
